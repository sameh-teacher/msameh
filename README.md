...
function submit(){
  var score = 0; var review = $('review'); review.innerHTML='';
  ...
  clearInterval(tmr); $('submitBtn').disabled=true;
  $('studentNameOut').textContent = document.getElementById('student').value || 'طالب بدون اسم';
  $('out').style.display='block';
  $('score').textContent = String(score);
  var band = (score>=54)?'ممتاز':(score>=48)?'جيد جدًا':(score>=36)?'جيد':(score>=30)?'مقبول':'ضعيف';
  $('band').textContent = 'التقدير: ' + band;
}
